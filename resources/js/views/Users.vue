...
import { userObjectIsEqual } from '../helpers/object-equal'
import {
  DropDownCompanies,
...
export default {
    data() {
        return {
            ...
            editID: null,
            columns: [
                // { field: 'UserID', filterable:false, editable: false, title: '#', width:'100px', locked: true},
                { field: 'UserName', title: 'Nom', filterable:true},
                { field: 'Password', title: 'Password'},
                { field: 'Company', title: 'Branch', filterable:true, cell: DropDownCompanies},
                { field: 'Admin', title: 'Admin', filterable:true},
    ...
    computed: {
        usersList() {
            return this.items.map((item) => Object.assign({ inEdit: item.UserID === this.editID}, item));
        },
        getData() {
    ...
    methods: {
        ...
        itemChange: function (e) {
            ...
            const index = data.findIndex(d => d.UserID === e.dataItem.UserID);
            ...
            const index2 = data2.findIndex(d => d.UserID === e.dataItem.UserID)

            if(!data[index].new) {
                if(userObjectIsEqual(data[index], data2[index2])) {
        ...
        rowClick: function (e) {
            if (this.editID) {
                const data = this.items.slice();
                const item = data.find(d => d.UserID == this.editID)
        ...
        validateRow(data) {
            if(data != undefined) {
                let cond = !!data.UserName &&
                    !!data.CompanyFID
                if(data.new) {
                    cond = cond && !!data.Password
                }
                return cond
            } else {
                return true
        ...
        alert() {
            this.$toasted.info('Veuillez remplir les champs obligatoires!!! ( Nom, Pass, Company)', {
                action : {
                text : 'Fermer',
