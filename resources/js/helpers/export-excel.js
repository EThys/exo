let myExcelXML = (function() {
    ...
    let fs, SheetName = 'SHEET 1',
        ...
        fileName = "Employee_List", uri, link, titles;

    class myExcelXML {
        constructor(o, file, titre) {
    ...
    const myXMLTable = function(o) {
        ...
        if (tableData.length > 0) {
            ...
            // const columnHeader = titles // by DON
            let rowData;
...
const saveExcel2 = function(data, fileName, columns) {
    let titles = []
    let arr = data.map(item => {
        let newItem = {}
        columns.forEach(col => {
            newItem[col.field] = item[col.field]
            let newTitle = col.title ? col.title : col.field
            if(!titles.includes(newTitle)) titles.push(newTitle)
        });
        return newItem
    })
    // console.log(arr)
    var myTestXML = new myExcelXML(JSON.stringify(arr), fileName, titles);
    myTestXML.downLoad();
}
